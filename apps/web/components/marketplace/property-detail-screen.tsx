...
const authPromptBenefits = [
  { Icon: MessageCircle, title: "Register Interest", description: "Our sales team can contact you about this property." },
  { Icon: MapPin, title: "Get the full address", description: "Shared when it is time to arrange a viewing." },
  { Icon: Heart, title: "Save what you like", description: "Keep properties in one place and compare later." }
] as const;
...
export function AuthPrompt({ action, onClose, trigger, returnTo }: { action: AuthPromptAction; onClose: () => void; trigger: React.RefObject<HTMLElement | null>; returnTo: string }) {
  ...
  return <div className="property-detail-modal-backdrop"><button type="button" className="property-detail-modal-dismiss" aria-label="Dismiss sign in prompt" onClick={onClose} /><div ref={dialog} className="property-detail-modal property-detail-auth-prompt" data-action={action} role="dialog" aria-modal="true" aria-labelledby="auth-prompt-title"><button className="property-detail-dialog-close" type="button" aria-label="Close sign in prompt" onClick={onClose}><X size={20} /></button><ShieldCheck size={42} aria-hidden="true" /><h2 id="auth-prompt-title">Set up a free account to continue</h2><p>Takes under a minute. We&apos;ll bring you right back to your property.</p><ul className="property-detail-auth-benefits">{authPromptBenefits.map(({ Icon, title, description }) => <li key={title}><Icon size={18} aria-hidden="true" /><span><strong>{title}</strong><small>{description}</small></span></li>)}</ul><div className="property-detail-modal-actions"><Link className="btn btn-primary" href={`/signup?returnTo=${encodeURIComponent(returnTo)}` as Route}>Create free account</Link><Link className="property-detail-login-link" href={loginHrefFor(returnTo) as Route}>I already have an account</Link></div></div></div>;
}
