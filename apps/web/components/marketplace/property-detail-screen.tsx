...
function AuthPrompt({ action, onClose, trigger, returnTo }: { action: AuthPromptAction; onClose: () => void; trigger: React.RefObject<HTMLElement | null>; returnTo: string }) {
  ...
  return <div className="property-detail-modal-backdrop"><button type="button" className="property-detail-modal-dismiss" aria-label="Dismiss sign in prompt" onClick={onClose} /><div ref={dialog} className="property-detail-modal" role="dialog" aria-modal="true" aria-labelledby="auth-prompt-title"><button className="property-detail-dialog-close" type="button" aria-label="Close sign in prompt" onClick={onClose}><X size={20} /></button><ShieldCheck size={38} aria-hidden="true" /><h2 id="auth-prompt-title">{title}</h2><p>Create an account or log in to save properties and contact our team about a listing.</p><div className="property-detail-modal-actions"><Link className="btn btn-primary" href={loginHrefFor(returnTo) as Route}>Log in</Link><Link className="btn btn-secondary" href={`/signup?returnTo=${encodeURIComponent(returnTo)}` as Route}>Create account</Link></div></div></div>;
}
