...
export function PublicSiteFooter(){return <footer className="site-footer"><div className="site-footer-grid">
  <section className="site-footer-about"><BrandLogo size={31}/><h2>About Us</h2><p>Since incorporation in 2001, Beryl Shelter Nigeria Limited has embarked on a number of strategic initiatives to expand the growth of its real estate market share in Nigeria and ensure strong continuous relevance.</p><small>© 2026 Beryl Shelter Nigeria Limited. All rights reserved.</small></section>
  {groups.map(([title,items])=><section key={title}><h2>{title}</h2>{items.map(item=><span key={item}>{item}</span>)}</section>)}
  <section className="site-footer-contact"><h2>Contact Enquiries</h2><p>Email: info@berylshelter.com<br/>Phone: 0704 205 5678<br/>Address: Plot 2, Cornerstone Estate Drive, Ikate-Elegshi, Lekki, Lagos</p></section>
