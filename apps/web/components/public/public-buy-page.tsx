...
const facilityOptions = ["Swimming Pool", "Balcony/Terrace", "Gym/Fitness Center", "CCTV", "Air Conditioning", "Laundry", "Garden", "Wi-Fi", "Housekeeping Services", "Car Park", "24Hrs Security"];
const unavailableFacilities = ["Children Play Area", "Tennis Court", "Basketball Court"];
const quickSubtypes = ["Block of flats", "Terraced Duplexes", "Terraced Bungalows", "Semi-Detached Bungalows", "Detached Bungalows", "Detached Duplexes"];
...
function FilterFields({ draft, setDraft, apply, reset, mobile = false, error }: {
  ...
  const countField = (label: string, key: "bedrooms" | "bathrooms") => <fieldset className="buy-count-filter"><legend>{label}</legend><div>
    ...
    <button type="button" disabled title="Seven or more is not yet supported">7+</button>
  </div></fieldset>;
  return <div className={`buy-filter-fields${mobile ? " mobile" : ""}`}>
    ...
    <div className="buy-filter-pair"><label>State<select value={draft.state} onChange={event => update("state", event.target.value)}><option value="">Select State</option><option>Lagos</option></select></label><label>City<input value={draft.city} maxLength={100} placeholder="Enter City" onChange={event => update("city", event.target.value)}/></label></div>
    ...
    <fieldset className="buy-facility-filter"><legend>Additional Conveniences</legend>
      {["Swimming Pool", "Balcony/Terrace", ...unavailableFacilities, ...facilityOptions.slice(2)].map((facility, index) => <label key={`${facility}-${index}`} className={unavailableFacilities.includes(facility) ? "unavailable" : ""}><input type="radio" name={mobile ? "buy-mobile-facility" : "buy-desktop-facility"} disabled={unavailableFacilities.includes(facility)} checked={draft.facility === facility} onChange={() => update("facility", facility)}/>{facility}{unavailableFacilities.includes(facility) && <small>Unavailable</small>}</label>)}
...
function PropertyCard({ property }: { property: PublicProperty }) {
  ...
  return <article className="buy-property-card"><div className="buy-property-image">
    ...
    <div className="buy-card-bottom"><span>Property Code: {property.code}</span>{notice && <small role="status">Saved properties are not available yet.</small>}</div>
  </div></article>;
}
...
export function PublicBuyPage() {
  ...
  function quickBedrooms(count: number) { apply({ ...draft, bedrooms: draft.bedrooms === String(count) ? "" : String(count) }); }
  function quickBungalow() { apply({ ...draft, propertySubtype: draft.propertySubtype === "Bungalow" ? "" : "Bungalow" }); }
  ...
  return <div className="public-page public-buy-page"><PublicHeader sessionAware mobileMenu accountMenu/><main>
    <div className="buy-top"><h1>Buy</h1>
      ...
      <div className="buy-mandate"><div><strong>Buy Mandate</strong><p>This is a document prepared by our Legal team which when signed will give Beryl right and privileges.</p></div><button type="button" onClick={() => setMandateNotice(true)}>Sign Mandate ↗</button></div>
      ...
      <div className="buy-quick"><p>Quick Filters</p><form className="buy-search" onSubmit={submitSearch}><label htmlFor="buy-q">Search properties</label><div><input id="buy-q" value={draft.q} maxLength={100} placeholder="Search by title, code, state or city" onChange={event => setDraft({ ...draft, q: event.target.value })}/><button type="submit">Search</button></div></form>
        <div className="buy-quick-chips">{[1,2,3,4,5,6].map(count => <button key={count} type="button" aria-pressed={draft.bedrooms === String(count)} onClick={() => quickBedrooms(count)}>{count} {count === 1 ? "Bedroom" : "Bedrooms"}</button>)}</div>
        <div className="buy-quick-chips"><button type="button" aria-pressed={draft.propertySubtype === "Bungalow"} onClick={quickBungalow}>Bungalow</button>{quickSubtypes.map((name, index) => <button key={`${name}-${index}`} type="button" disabled title="This property subtype is not available in current listings">{name}</button>)}</div>
    ...
    <div className="buy-results-band"><div className="buy-results-layout"><section className="buy-results" aria-label="Properties for sale">
      <div className="buy-results-toolbar"><p>{loading ? "Loading results…" : `${first}–${last} of ${total} ${total === 1 ? "property" : "properties"} for sale`}</p><span>↕ &nbsp; Sort: Latest listed</span></div>
