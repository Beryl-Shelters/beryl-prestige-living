...
export function BuyerOnboardingScreen() {
  ...
  const complete = async (body: Parameters<typeof customerApi.buyerOnboarding>[0]) => { setError(""); try { await mutation.mutateAsync(body); const refreshed = await refreshSession(); router.replace(routeForNextAction(refreshed.nextAction)); } catch (caught) { setError(apiErrorOf(caught).message); } };
