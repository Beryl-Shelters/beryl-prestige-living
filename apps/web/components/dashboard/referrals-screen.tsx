  ...
  return <section className="referrals-page">
    ...
    <div className="referrals-intro">
      <section className="dashboard-card referrals-earn"><h2>Earn with Beryl Shelter</h2><p>Invite your friends to Beryl Shelter and earn {rate}% of every property purchase they make through our platform. Start referring today and benefit from their investments!</p>
        <div className="referrals-flow">{[["send","Send Invitation","Refer a property to friends and potential buyers and tell them about Beryl Shelter"],["register","Registration and Purchase","Your referrals should register to our platform using your personal referral code"],["reward","Referral Reward",`Once they purchase a property through our platform you get a ${rate}% bonus on their purchase`]].map(([kind,title,copy],index)=><div key={title} className="referral-flow-step"><FlowIcon kind={kind as "send"|"register"|"reward"}/>{index<2&&<i aria-hidden="true"/>}<h3>{title}</h3><p>{copy}</p></div>)}</div>
      </section>
      <section className="dashboard-card referrals-invite"><h2>Invite your friends</h2><p>Invite your friends by referring properties they might want to purchase or by encouraging them to list their properties for sale with us.</p><div className="referral-actions"><div><span>Refer a property to a friend?</span><Link href="/buy">View Properties</Link></div><div><span>Know someone who has a property to sell?</span><button type="button" disabled={creating} onClick={()=>void referSeller()}>Refer a Friend to Sell</button></div></div><p>Earn <strong>{rate}% commission</strong> for every property purchased through your referral.</p></section>
    </div>
