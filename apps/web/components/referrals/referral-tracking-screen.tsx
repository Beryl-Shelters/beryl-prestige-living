...
export function ReferralTrackingScreen() {
  ...
  const request = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      setError(value.response?.data?.code === "REFERRAL_TRACKING_UNAVAILABLE" ? "WhatsApp/SMS tracking codes are not configured yet. Please keep your referral reference safe." : value.response?.data?.message || "We could not request a tracking code.");
  ...
  return <main className="referral-tracking-page"><section className="referral-access-card">
    ...
    <p>{requested ? "Enter the six-digit code sent to your phone." : "Enter the name and phone number used for your referral. We’ll send a code on WhatsApp or SMS when a provider is available."}</p>
