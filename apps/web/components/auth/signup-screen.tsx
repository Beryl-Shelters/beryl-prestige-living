...
export function SignupScreen() {
  ...
  const form = useForm<SignupValues>({ resolver: zodResolver(signupSchema), defaultValues: { gettingStartedAs: "FIND_PROPERTY", fullName: "", email: "", phone: "+234", isWhatsAppNumber: true, whatsAppNumber: "", password: "", confirmPassword: "" } });
  ...
  const submit = form.handleSubmit(async (values) => {
    setGlobalError("");
    try {
      const response = await mutation.mutateAsync({ ...values, email: values.email.trim().toLowerCase(), phone: normalizePhone(values.phone), whatsAppNumber: values.isWhatsAppNumber ? null : normalizePhone(values.whatsAppNumber ?? "") });
  ...
  return <AuthShell intent={intent} backHref="/"><h1 className="page-title">Create your account</h1><p className="page-copy">To save homes and message agents, set up a free account. Takes a minute.</p><form className="form-stack" onSubmit={submit} noValidate><fieldset><legend className="field-label mb-2">How would you like to get started?</legend><div className="choice-grid"><button type="button" className="choice-card" data-selected={intent === "FIND_PROPERTY"} aria-pressed={intent === "FIND_PROPERTY"} onClick={() => form.setValue("gettingStartedAs", "FIND_PROPERTY")}><Search size={17} aria-hidden />Find a property</button><button type="button" className="choice-card" data-selected={intent === "LIST_PROPERTY"} aria-pressed={intent === "LIST_PROPERTY"} onClick={() => form.setValue("gettingStartedAs", "LIST_PROPERTY")}><Home size={17} aria-hidden />List a property</button></div></fieldset><InputField label="Full name" autoComplete="name" error={form.formState.errors.fullName?.message} {...form.register("fullName")} /><InputField label="Email address" type="email" autoComplete="email" error={form.formState.errors.email?.message} {...form.register("email")} /><InputField label="Phone number" inputMode="tel" autoComplete="tel" error={form.formState.errors.phone?.message} {...form.register("phone")} /><fieldset><legend className="field-label mb-2">Is this your WhatsApp number?</legend><div className="choice-grid"><button type="button" className="choice-card" data-selected={isWhatsApp} aria-pressed={isWhatsApp} onClick={() => form.setValue("isWhatsAppNumber", true)}>Yes</button><button type="button" className="choice-card" data-selected={!isWhatsApp} aria-pressed={!isWhatsApp} onClick={() => form.setValue("isWhatsAppNumber", false)}>No</button></div></fieldset>{!isWhatsApp ? <InputField label="WhatsApp number" inputMode="tel" autoComplete="tel" error={form.formState.errors.whatsAppNumber?.message} {...form.register("whatsAppNumber")} /> : null}<PasswordField label="Password" autoComplete="new-password" error={form.formState.errors.password?.message} {...form.register("password")} />{password ? <PasswordStrength password={password} /> : null}<PasswordField label="Confirm password" autoComplete="new-password" error={form.formState.errors.confirmPassword?.message} {...form.register("confirmPassword")} />{globalError ? <ApiAlert>{globalError}</ApiAlert> : null}<p className="text-xs leading-5 text-brand-muted">By creating an account, you agree to our <Link className="text-blue-600 underline" href="/privacy">Privacy Policy</Link> and <Link className="text-blue-600 underline" href="/terms">Terms and Conditions</Link>.</p><button className="btn btn-primary w-full" disabled={mutation.isPending} type="submit">{mutation.isPending ? <><Spinner label="Creating account" />Creating account…</> : "Create account"}</button><p className="text-center text-sm">Already have an account? <Link className="font-bold text-brand-brown" href="/login">Log in</Link></p></form></AuthShell>;
}
