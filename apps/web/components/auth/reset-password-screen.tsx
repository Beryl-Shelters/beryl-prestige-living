...
import { apiErrorOf, friendlyPasswordResetError } from "@/lib/api/errors";
...
export function ResetPasswordScreen() {
  ...
  const submit = form.handleSubmit(async (values) => {
    ...
    try { await mutation.mutateAsync(values); router.replace("/login"); }
      ...
      const failure = apiErrorOf(caught);
      setMustRestart(["INVALID_RESET_TOKEN", "RESET_TOKEN_EXPIRED", "RESET_TOKEN_USED"].includes(failure.code ?? ""));
      setError(friendlyPasswordResetError(failure));
    }
  });
  return <main className="min-h-svh bg-brand-canvas p-5 md:p-10"><Link className="eyebrow-link" href="/forgot-password"><ArrowLeft size={14} />Back</Link><section className="mx-auto mt-[7svh] w-full max-w-md rounded-card bg-white p-6 shadow-card md:p-10"><div className="mx-auto mb-5 grid h-20 w-20 place-items-center rounded-full bg-brand-cream"><LockKeyhole size={40} color="var(--color-brand-brown)" /></div><h1 className="page-title">Set a new password</h1><p className="page-copy">Choose a new password for your account. Make it one you&apos;ll remember.</p><form className="form-stack" onSubmit={submit}><PasswordField label="New password" autoComplete="new-password" disabled={mustRestart} error={form.formState.errors.newPassword?.message} {...form.register("newPassword")} />{form.watch("newPassword") ? <PasswordStrength password={form.watch("newPassword")} /> : null}<PasswordField label="Confirm new password" autoComplete="new-password" disabled={mustRestart} error={form.formState.errors.confirmPassword?.message} {...form.register("confirmPassword")} /><ApiAlert tone="info">For your security, you will be logged out on all devices and need to log in again.</ApiAlert>{error ? <ApiAlert>{error}</ApiAlert> : null}{mustRestart ? <Link className="btn btn-primary" href="/forgot-password">Start password reset again</Link> : <button className="btn btn-primary" disabled={mutation.isPending}>{mutation.isPending ? <><Spinner />Saving…</> : "Save new password"}</button>}</form></section></main>;
}
