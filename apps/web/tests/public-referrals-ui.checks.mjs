...
export async function checkPublicReferrals({ page, origin, calls, failures, screenshot, passed }) {
  ...
  for (const width of [1440, 1280, 768, 430, 390, 360]) {
    ...
    for (const image of await cards.locator("img").all()) assert.equal(await image.evaluate(img => img.complete && img.naturalWidth > 0), true);
  ...
  assert.deepEqual(await cards.locator("a").evaluateAll(links => links.map(link => new URL(link.href).pathname + new URL(link.href).search)),
    ["/login?next=/dashboard/listings", "/login?next=/dashboard/referrals"]);
  await cards.first().locator("a").click();
  await page.waitForURL(url => url.pathname === "/login" && url.searchParams.get("next") === "/dashboard/listings");
  await page.goto(origin + "/referrals");
  ...
  failures.delete("/me");
  await page.goto(origin + "/referrals");
  await page.waitForFunction(() => document.querySelector(".public-referrals-card a")?.getAttribute("href") === "/dashboard/listings");
  assert.deepEqual(await cards.locator("a").evaluateAll(links => links.map(link => new URL(link.href).pathname)),
    ["/dashboard/listings", "/dashboard/referrals"]);
  await cards.first().locator("a").click();
  ...
  passed.push("Public Referrals matches two-column desktop and stacked mobile design at six widths, with working supplied icon and no overflow");
  passed.push("Buy and Seller CTAs use distinct existing login/dashboard routes; viewing or clicking does not create a referral");
}
