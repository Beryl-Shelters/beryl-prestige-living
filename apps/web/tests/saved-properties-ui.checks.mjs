...
export const savedPropertiesState = { items: structuredClone(fixture) };
export function resetSavedPropertiesState() { savedPropertiesState.items = structuredClone(fixture); }
...
export async function checkSavedProperties({page,origin,calls,failures,screenshot,passed}) {
  ...
  assert.equal(await page.locator(".saved-property-card").count(),3);
  ...
  assert.equal(await page.getByLabel(/Remove .* from saved properties/).count(),3);
  assert.equal(await page.getByLabel(/Share /).count(),3);
  ...
  for(const label of ["Compare Property","Mortgage Calculator"]){const item=page.locator(".public-account-dropdown").getByRole("button",{name:label});assert.equal(await item.getAttribute("aria-disabled"),"true");}
  ...
  passed.push("Desktop/tablet/mobile layouts match the supplied grid/stack structure at eight widths; account dropdown and existing Buy Save entry point are active while Compare and Mortgage remain unavailable");
}
