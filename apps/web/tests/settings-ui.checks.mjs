...
export function settingsRequestBody(request){
  ...
  return {...JSON.parse(data?.[1]??"{}"),_hasImage:raw.includes('name="profileImage"'),_imageMime:mime?.[1]};
}
export function settingsResponse(method,body){
  if(method==="GET")return structuredClone(settingsState.profile);
  const {_hasImage,_imageMime,...data}=body,extension=_imageMime==="image/png"?"png":_imageMime==="image/jpeg"?"jpg":"webp";
  Object.assign(settingsState.profile,data,_hasImage?{profileImageUrl:`https://images.example.test/profile.${extension}`}:{});
  dashboardFixture.customer.profile_image_url=settingsState.profile.profileImageUrl;
  return structuredClone(settingsState.profile);
}

export async function checkSettings({page,origin,calls,failures,screenshot,passed,pauseRequest,resume,toast}){
  const profileEndpoint="/dashboard/settings/profile",passwordEndpoint="/dashboard/settings/password";
  ...
  const closeError=async()=>{const item=page.locator("#settings-profile-error");if(await item.count()){await item.locator(".Toastify__close-button").click();await item.waitFor({state:"detached"});}};
  for(const width of [1440,1280,1024,768,390,320]){
    ...
    assert.equal(await page.getByRole("tab",{name:"Business"}).isDisabled(),true);
