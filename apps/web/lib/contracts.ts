...
export type DirectReferralRequest = {
  ...
  referred: { fullName: string; contactMethod: ReferralContactMethod; phone?: string; email?: string };
...
export type DirectReferralResult = {
  referral: { id: string; referenceId: string; referredFirstName: string; purpose: ReferralPurpose; status: "NEW"; submittedAt: string };
...
export type ReferralDashboardItem = {
  id: string; referenceId: string; referredName: string; purpose: ReferralPurpose;
  contactMethod: ReferralContactMethod; status: ReferralLifecycle; statusLabel: string;
  rewardAmount: number | null; paymentStatus: ReferralPaymentStatus; submittedAt: string;
};
export type ReferralDashboard = {
  referrer: { fullName: string; referralCode: string; referralLink: string };
  summary: { referralCount: number; completedCount: number; earnedAmount: number; outstandingAmount: number };
  referrals: ReferralDashboardItem[];
  pagination: { page: number; limit: number; total: number; totalPages: number };
};
export type ReferralPayoutDetails = { payoutDetails: null | { bankCode: string; bankName: string; accountName: string; maskedAccountNumber: string; updatedAt: string } };
export type ReferralBankDirectory = { banks: { code: string; name: string }[]; authoritativeCompleteDirectory: false; accountNameResolutionAvailable: false };
