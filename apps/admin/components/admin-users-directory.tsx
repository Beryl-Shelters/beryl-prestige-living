...
const roleTabs = [
  ["", "All", "totalUsers"],
  ["BUYER", "Buyers", "buyerProfiles"],
  ["SELLER", "Sellers", "sellerProfiles"],
  ["REFERRER", "Referrers", "referrerProfiles"]
] as const;
...
export function AdminUsersDirectory() {
  ...
  return <section className="admin-users" aria-labelledby="users-title">
    ...
    <div className="users-toolbar">
      ...
      <details className="users-filter"><summary><Filter size={15} aria-hidden />Filter by</summary><div><label>Verification<select value={verification} onChange={(event) => { setVerification(event.target.value); setPage(1); }}><option value="">All statuses</option><option value="VERIFIED">Verified</option><option value="UNVERIFIED">Unverified</option></select></label></div></details>
    ...
    <div className="user-role-tabs" role="tablist" aria-label="Customer profile">
      {roleTabs.map(([value, label, countKey]) => <button role="tab" aria-selected={role === value} type="button" key={label} onClick={() => { setRole(value); setPage(1); }}>{label}<span>{data?.counts[countKey] ?? 0}</span></button>)}
    </div>
    ...
    <div className="users-table-card">
      ...
      {!loading && !error && data?.items.length === 0 ? <div className="users-empty"><UsersRound size={26} aria-hidden /><strong>{query || role || verification ? "No users match these filters" : "No customers yet"}</strong><p>{query || role || verification ? "Try changing your search or filters." : "Registered customers will appear here."}</p></div> : null}
