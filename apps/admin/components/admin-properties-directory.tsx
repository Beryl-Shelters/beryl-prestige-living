...
export function AdminPropertiesDirectory() {
  ...
  const filtered = Boolean(query || category || mandate);
  const emptyCopy = filtered
    ? ["No properties match these filters", "Try changing the search, status, or filters."]
    : status === "IN_REVIEW"
      ? ["Queue’s clear", "No properties waiting for review."]
      : status === "LIVE"
        ? ["Nothing here yet", "No approved properties so far."]
        : status === "REJECTED"
          ? ["No rejected properties", "Rejected properties will appear here."]
          : ["No properties yet", "Submitted properties will appear here."];
  ...
  return <section className="admin-properties" aria-labelledby="properties-title">
    ...
    <div className="properties-table-card"><div className="properties-table-scroll"><table className="properties-table"><thead><tr><th>ID</th><th>Property</th><th>Category</th><th>Mandate</th><th>Location</th><th>Status</th><th>Actions</th></tr></thead><tbody>
      {loading ? Array.from({ length: 6 }, (_, index) => <tr key={index} aria-label="Loading property"><td colSpan={7}><span className="property-row-skeleton" /></td></tr>) : null}
      {!loading && !error ? data?.items.map((property) => {
        ...
        return <tr key={property.id}><td><span className="property-reference-cell">{property.referenceId}</span></td><td><div className="property-name-cell"><span className="property-row-image" style={property.coverImage ? { backgroundImage: `url(${property.coverImage.url})` } : undefined}><Building2 size={16} /></span><div><strong>{property.title || "Untitled property"}</strong><small>{formatPropertyDate(relevantDate)}</small></div></div></td><td><span className="property-category-badge">{humanizePropertyValue(property.propertyCategory)}</span></td><td><span className={`property-mandate-badge ${property.mandateType?.toLowerCase() || "none"}`}>{humanizePropertyValue(property.mandateType)}</span></td><td>{property.publicLocation || "Not available"}</td><td><span className={`property-status status-${mappedStatus.tone}`}>{mappedStatus.label}</span></td><td><Link className="property-row-action" href={`/dashboard/properties/${property.id}` as never} aria-label={`View details for ${property.title || property.referenceId}`} title="View details"><MoreVertical size={17} /></Link></td></tr>;
      }) : null}
    </tbody></table></div>
    {!loading && !error && data?.items.length === 0 ? <div className="properties-empty"><Building2 size={28} /><strong>{emptyCopy[0]}</strong><p>{emptyCopy[1]}</p></div> : null}
