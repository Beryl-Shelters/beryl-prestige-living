...
export function ActivationOtpScreen() {
  const router = useRouter(); const [challenge] = useState<Challenge | null>(() => typeof window === "undefined" ? null : read()); const [remaining, setRemaining] = useState(() => typeof window === "undefined" ? 0 : read()?.resendAvailableIn ?? 0); const [busy, setBusy] = useState(false); const [message, setMessage] = useState(""); const [resetKey, setResetKey] = useState(0); const [resendCount, setResendCount] = useState(0); const viewed = useRef(false);
  useEffect(() => { if (!challenge) router.replace("/login"); else if (!viewed.current) { viewed.current = true; void trackAdminEvent("Verification Screen Viewed", { otp_context: "admin_activation" }); } }, [challenge, router]);
