...
describe("protected Admin property detail from Leads", () => {
  ...
  it("renders the property header and mapped lifecycle status", () => { expect(detail).toContain("property-detail-header"); expect(detail).toContain("propertyStatus[summary.status]"); expect(detail).toContain("summary.referenceId"); });
  it("renders supported operational property details", () => ["Full Address", "Category", "Type", "Condition", "Bedrooms", "Bathrooms", "Toilets", "Parking"].forEach((label) => expect(detail).toContain(label)));
  ...
  it("requests document access only after explicit View or Download", () => { expect(detail).toContain('accessDocument(item.id, item.displayName, "view")'); expect(detail).toContain('accessDocument(item.id, item.displayName, "download")'); expect(documentBff).toContain("documents/${encodeURIComponent(documentId)}/access"); expect(detail).not.toContain("item.url"); });
  it("allows only a Lead detail return path and otherwise returns to Properties", () => { expect(safeAdminPropertyReturnPath(`/dashboard/leads/${leadId}`)).toBe(`/dashboard/leads/${leadId}`); expect(safeAdminPropertyReturnPath("https://evil.example/path")).toBe("/dashboard/properties"); expect(safeAdminPropertyReturnPath("/dashboard/properties/other")).toBe("/dashboard/properties"); });
