...
describe("Admin Properties approved interface architecture", () => {
  ...
  it("keeps the exact sidebar and enables Properties", () => { ["Dashboard", "Users", "Properties", "Leads", "Referrers", "Admin Management", "Settings", "Log out"].forEach((label) => expect(shell).toContain(label)); expect(shell).toContain('admin.adminRole === "SUPER_ADMIN"'); ["My Listings", "Payments", "Subaccounts", "Save-as-you-earn", "Invest", "Refer & Earn", "Support"].forEach((label) => expect(shell).not.toContain(label)); expect(shell).toContain('href={"/dashboard/properties" as never}'); });
  ...
  it("renders the updated seven approved table columns including ID", () => { ["ID", "Property", "Category", "Mandate", "Location", "Status", "Actions"].forEach((label) => expect(directory).toContain(`<th>${label}</th>`)); expect(directory).toContain("property.referenceId"); });
  ...
  it("supports updated lifecycle empty states, filtered-empty, skeleton, and retry", () => { ["property-row-skeleton", "No properties yet", "No properties match these filters", "Queue’s clear", "No properties waiting for review.", "Nothing here yet", "No approved properties so far.", "Try again"].forEach((value) => expect(directory).toContain(value)); });
  ...
  it("shows authoritative approved and rejected operational side states", () => {
    ["Live on the marketplace", "View on the marketplace", "NEXT_PUBLIC_PUBLIC_WEB_URL", "reviewedByAdminName", "Awaiting Seller", "Earlier Attempts", "rejectionAttempts"].forEach((value) => expect(detail).toContain(value));
  });
