...
describe("Admin BFF security architecture", () => {
  ...
  it("provides BFF session lifecycle handlers without returning tokens to the browser", () => { const logout = source("app/api/admin/logout/route.ts"); expect(source("app/api/admin/refresh/route.ts")).toContain("setAdminSession"); expect(logout).toContain("admin/auth/logout"); expect(logout.match(/clearAdminCookies\(next\)/g)).toHaveLength(2); expect(source("app/api/admin/change-password/route.ts")).toContain("clearAdminCookies"); expect(source("app/api/admin/_shared.ts")).toContain("admin/auth/refresh"); });
});
