...
describe("Admin BFF security architecture", () => {
  ...
  it("provides BFF session lifecycle handlers without returning tokens to the browser", () => { expect(source("app/api/admin/refresh/route.ts")).toContain("setAdminSession"); expect(source("app/api/admin/logout/route.ts")).toContain("admin/auth/logout"); expect(source("app/api/admin/change-password/route.ts")).toContain("clearAdminCookies"); expect(source("app/api/admin/_shared.ts")).toContain("admin/auth/refresh"); });
});
