...
describe("mobile persona switching",()=>{
  ...
  it("activates unavailable personas and refreshes canonical status before routing",()=>{const switcher=source("components/persona-switcher.tsx");expect(switcher).toContain("activating ? await activatePersona");expect(switcher).toContain(": await switchPersona");expect(switcher).toContain("await refreshOnboardingStatus()");expect(switcher).toContain("onNavigate(nextAction)");});
});
