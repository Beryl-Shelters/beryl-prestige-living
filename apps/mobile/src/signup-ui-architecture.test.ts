...
describe("signup mobile UI regressions",()=>{
  ...
  it("makes only the signup Log in text interactive and brown",()=>{const signup=source("components/signup-form.tsx");expect(signup).toContain('accessibilityLabel="Log in"');expect(signup).toContain('router.push("/login")');expect(signup).toMatch(/color:\s*colors\.ink/);expect(signup).toMatch(/color:\s*colors\.brown/);});
});
