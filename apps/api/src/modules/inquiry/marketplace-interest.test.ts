...
const inquiry={id:"inquiry-1",property_id:propertyId,inquiry_type:"MARKETPLACE_INTEREST_WHATSAPP",status:"pending",created_at:"2026-08-18T18:00:00.000Z",email:"private",phone_number:"private",message:"private"};
...
describe("Marketplace Buyer interest through canonical inquiries",()=>{
  ...
  it.each(["WHATSAPP","CALL","EMAIL"] as const)("accepts %s using authenticated profile contacts",async(preferredContactMethod)=>{
    const result=await createMarketplaceInterest(propertyId,customerId,{preferredContactMethod,message:"Please share more details"});
    expect(result).toEqual({inquiryId:"inquiry-1",propertyId,referenceId:"BRL-LIVE",title:"Four bedroom duplex",askingPrice:250000000,preferredContactMethod,submittedAt:"2026-08-18T18:00:00.000Z",nextAction:"KEEP_BROWSING"});
    const inserted=database.calls.find(call=>call.table==="inquiries"&&call.method==="insert")?.args[0] as any;
    expect(inserted).toMatchObject({user_id:customerId,property_id:propertyId,inquiry_type:`MARKETPLACE_INTEREST_${preferredContactMethod}`,status:"pending",full_name:"Buyer Customer",email:"buyer@example.com",message:"Please share more details"});
    expect(inserted.phone_number).toBe(preferredContactMethod==="WHATSAPP"?"+2348098765432":"+2348012345678");
  ...
  it("enforces authoritative LIVE status without legacy publication flags",async()=>{
    await createMarketplaceInterest(propertyId,customerId,{preferredContactMethod:"EMAIL"});
  ...
  it.each(["DRAFT","IN_REVIEW","REJECTED","LEGACY_APPROVED"])("makes %s unavailable safely",async()=>{
    ...
    await expect(createMarketplaceInterest(propertyId,customerId,{preferredContactMethod:"EMAIL"})).rejects.toMatchObject({statusCode:404,code:"PROPERTY_NOT_AVAILABLE",message:"Property is not available"});
  ...
  it.each([
    ...
    ["EMAIL",{email:null}]
  ] as const)("returns a safe error when %s is unavailable",async(preferredContactMethod,profileOverride)=>{
    prepare(profileOverride);
    await expect(createMarketplaceInterest(propertyId,customerId,{preferredContactMethod})).rejects.toMatchObject({statusCode:409,code:"CONTACT_METHOD_UNAVAILABLE",message:"Preferred contact method is unavailable"});
  });

  it("accepts an omitted message and stores an internal non-SLA fallback",async()=>{
    await createMarketplaceInterest(propertyId,customerId,{preferredContactMethod:"CALL"});
    const inserted=database.calls.find(call=>call.table==="inquiries"&&call.method==="insert")?.args[0] as any;
  ...
  it("validates contact method, trims messages, normalizes blanks, limits size, and rejects client identity fields",()=>{
    expect(marketplaceInterestSchema.parse({preferredContactMethod:"EMAIL",message:"  Details please  "})).toEqual({preferredContactMethod:"EMAIL",message:"Details please"});
    expect(marketplaceInterestSchema.parse({preferredContactMethod:"CALL",message:"   "})).toEqual({preferredContactMethod:"CALL",message:undefined});
    expect(marketplaceInterestSchema.safeParse({preferredContactMethod:"SMS"}).success).toBe(false);
    expect(marketplaceInterestSchema.safeParse({preferredContactMethod:"EMAIL",message:"x".repeat(1001)}).success).toBe(false);
    expect(marketplaceInterestSchema.safeParse({preferredContactMethod:"EMAIL",customerId:"attacker",email:"attacker@example.com"}).success).toBe(false);
  });
  ...
  it("allows explicit later inquiry submissions as separate existing-domain rows",async()=>{
    ...
    const first=await createMarketplaceInterest(propertyId,customerId,{preferredContactMethod:"EMAIL"});
    const second=await createMarketplaceInterest(propertyId,customerId,{preferredContactMethod:"EMAIL"});
  ...
  it("returns stable safe persistence errors",async()=>{
    ...
    await expect(createMarketplaceInterest(propertyId,customerId,{preferredContactMethod:"EMAIL"})).rejects.toMatchObject({statusCode:503,code:"INTEREST_SUBMISSION_FAILED"});
    prepare();database.queues.inquiries=[{data:null,error:{message:"private insert error"}}];
    await expect(createMarketplaceInterest(propertyId,customerId,{preferredContactMethod:"EMAIL"})).rejects.toMatchObject({statusCode:503,code:"INTEREST_SUBMISSION_FAILED",message:"Interest submission is temporarily unavailable"});
  });
});
