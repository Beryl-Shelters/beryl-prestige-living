import { z } from "zod";
import { marketplacePropertyTypes } from "./marketplace.contract";
const nullableNumber=z.number().finite().nonnegative().nullable().optional();
const nullableCount=z.number().finite().int().nonnegative().nullable().optional();
...
},z.array(z.enum(marketplacePropertyTypes)).min(1).max(marketplacePropertyTypes.length).optional());
