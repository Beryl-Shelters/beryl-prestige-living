...
describe("Marketplace rejected correction Swagger",()=>{
  ...
  it("documents the deterministic reopen response",()=>{const schema=specification.components.schemas.MarketplaceReopenResult;expect(schema.properties.status.enum).toEqual(["DRAFT"]);expect(schema.properties.nextAction.enum).toEqual(["EDIT_REJECTED_LISTING"]);expect(Object.keys(specification.paths)).toHaveLength(128)});
});
