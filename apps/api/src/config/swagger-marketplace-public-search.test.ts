...
describe("public Marketplace search Swagger",()=>{
  ...
  it("documents anonymous browsing with optional saved-state authentication",()=>{expect(operation.security).toEqual([{}, {bearerAuth:[]}]);expect(operation.description).toMatch(/public Buyer-safe LIVE.*optional customer session.*saved_properties/i)});
