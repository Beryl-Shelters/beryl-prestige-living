...
describe("public Marketplace search Swagger",()=>{
  ...
  it("documents anonymous LIVE-only browsing",()=>{expect(operation.security).toEqual([]);expect(operation.description).toMatch(/public anonymous.*marketplace_status=LIVE.*DRAFT.*IN_REVIEW.*REJECTED.*excluded/i);expect(Object.keys(specification.paths)).toHaveLength(130)});
