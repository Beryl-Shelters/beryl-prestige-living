...
const components = {
  ...
  schemas: {
    ...
    AdminMarketplaceReviewHistory: objectSchema({ id: uuid, previousStatus: { type: "string" }, newStatus: { type: "string", enum: ["LIVE", "REJECTED"] }, action: { type: "string", enum: ["APPROVED", "REJECTED"] }, reason: nullableString, reviewedByAdminId: uuid, reviewedByAdminName: nullableString, createdAt: dateTime }, ["id", "previousStatus", "newStatus", "action", "reason", "reviewedByAdminId", "reviewedByAdminName", "createdAt"]),
    ...
    AdminLeadCard: objectSchema({ id: uuid, referenceId: { type: "string", example: "ENQ-1036A9C2" }, customerName: { type: "string" }, propertyId: { ...uuid, nullable: true }, propertyTitle: nullableString, propertyReferenceId: nullableString, stage: ref("AdminLeadStage"), inquiryType: { type: "string" }, receivedAt: dateTime, source: { type: "string", enum: ["REFERRAL"], nullable: true }, referredBy: { type: "object", nullable: true, additionalProperties: false, properties: { id: uuid, fullName: { type: "string" } }, required: ["id", "fullName"] } }, ["id", "referenceId", "customerName", "stage", "inquiryType", "receivedAt", "source", "referredBy"]),
    ...
    AdminLeadDetail: { type: "object", description: "Operational Admin lead detail. Referral source is derived from the authoritative referral-to-inquiry relationship, and referredBy contains only bounded referrer identity. Customer contact information is limited to the inquiry/customer profile; referral secrets, payout data, Seller-private documents, and full addresses are excluded.", additionalProperties: true },
    ...
    NigeriaLocationSearchResult: objectSchema({
      locations: {
        type: "array",
        maxItems: 12,
        items: objectSchema({
          ...
          label: { type: "string" },
          state: { type: "string" },
          type: { type: "string", enum: ["STATE", "CITY", "LGA", "AREA"] }
        }, ["id", "label", "state", "type"])
      }
    }, ["locations"]),
...
const endpoints: Endpoint[] = [
  { method: "get", path: "/locations/search", tag: "Location Search", summary: "Search Nigerian locations", description: "Public bounded search for Nigerian states, cities, LGAs, and areas. The server queries GeoNames with country=NG, returns only a safe normalized DTO, caches repeated searches, coalesces concurrent identical requests, and applies an IP rate limit. Provider credentials and raw provider fields are never returned.", successMessage: "Nigeria locations fetched successfully", responseSchema: "NigeriaLocationSearchResult", security: "none", query: [queryParameter("q", "Trimmed location prefix. At least two and at most 80 characters.", { type: "string", minLength: 2, maxLength: 80 }, "Ile")], errorResponses: { "400": { description: "Search text is missing, too short, or too long", message: "Enter at least two characters to search", code: "INVALID_LOCATION_QUERY" }, "429": { description: "Public search request limit exceeded", message: "Too many requests, please try again later", code: "LOCATION_SEARCH_RATE_LIMITED" }, "503": { description: "Provider is unconfigured, unavailable, malformed, or timed out", message: "Location suggestions are temporarily unavailable", code: "LOCATION_SEARCH_UNAVAILABLE" } } },
  { method: "get", path: "/admin/leads", tag: "Admin Lead Management", summary: "List operational inquiry leads by stage", description: "Isolated Admin-session operation for ADMIN and SUPER_ADMIN. Returns database-authoritative counts and a bounded newest-first set per NEW, CONTACTED, WON, and LOST stage. Referral source and bounded referrer identity are derived from the authoritative referral-to-inquiry relationship. Optional q search is parameterized and matches customer name, property title/reference, inquiry UUID, or deterministic enquiry reference.", successMessage: "Admin leads fetched successfully", responseSchema: "AdminLeadList", security: "required", roles: ["ADMIN", "SUPER_ADMIN"], query: [queryParameter("q", "Trimmed lead search; blank means no filter.", { type: "string", maxLength: 120 }), queryParameter("limit", "Maximum cards returned per stage.", { type: "integer", minimum: 1, maximum: 50, default: 20 })], errorResponses: { "400": { description: "Invalid search or per-stage limit", message: "Invalid lead search or limit", code: "INVALID_LEAD_FILTER" }, "503": { description: "Lead persistence unavailable", message: "Lead management is temporarily unavailable", code: "LEADS_UNAVAILABLE" } } },
  ...
  { method: "post", path: "/auth/reset-password", tag: "Authentication", summary: "Reset a customer password", description: "Atomically consumes a verified, unexpired reset proof, increments the customer's session version, and revokes every customer session before the API updates the password through server-only Supabase Admin Auth. The API never writes managed Auth password columns directly.", successMessage: "Password reset successfully. Please log in with your new password.", responseSchema: "SessionsInvalidatedResult", bodySchema: "ResetCustomerPasswordRequest", successExample: { sessionsInvalidated: true, nextAction: "LOGIN" }, errorResponses: { "400": { description: "Password policy, confirmation, or same-password check failed", message: "New password does not meet the password policy", code: "PASSWORD_POLICY_INVALID", examples: { policy: { summary: "Supabase password policy rejected the password", message: "New password does not meet the password policy", code: "PASSWORD_POLICY_INVALID" }, same: { summary: "Password is unchanged", message: "New password must differ from current password", code: "NEW_PASSWORD_SAME_AS_CURRENT" } } }, "401": { description: "Reset proof is invalid or expired", message: "Invalid password-reset token", code: "INVALID_RESET_TOKEN", examples: { invalid: { summary: "Invalid reset proof", message: "Invalid password-reset token", code: "INVALID_RESET_TOKEN" }, expired: { summary: "Expired reset proof", message: "Password-reset token has expired", code: "RESET_TOKEN_EXPIRED" } } }, "409": { description: "Reset proof was already consumed", message: "Password-reset token has already been used", code: "RESET_TOKEN_USED" }, "503": { description: "Password reset is unavailable", message: "Password reset is temporarily unavailable", code: "PASSWORD_RESET_UNAVAILABLE" } } },
  ...
  { method: "patch", path: "/auth/change-password", tag: "Authentication", summary: "Change the current customer's password", description: "Requires a verified active customer session, verifies the current credentials through Supabase Auth, revokes every Beryl customer session, and updates the password through server-only Supabase Admin Auth. The API never writes managed Auth password columns directly.", successMessage: "Password changed successfully. Please log in again.", responseSchema: "SessionsInvalidatedResult", bodySchema: "ChangeCustomerPasswordRequest", security: "required", successExample: { sessionsInvalidated: true, nextAction: "LOGIN" }, errorResponses: { "400": { description: "Password policy, confirmation, or same-password check failed", message: "New password does not meet the password policy", code: "PASSWORD_POLICY_INVALID", examples: { policy: { summary: "Supabase password policy rejected the password", message: "New password does not meet the password policy", code: "PASSWORD_POLICY_INVALID" }, same: { summary: "Password is unchanged", message: "New password must differ from current password", code: "NEW_PASSWORD_SAME_AS_CURRENT" } } }, "401": { description: "Customer session is invalid or current password is incorrect", message: "Current password is incorrect", code: "CURRENT_PASSWORD_INCORRECT", examples: { password: { summary: "Incorrect current password", message: "Current password is incorrect", code: "CURRENT_PASSWORD_INCORRECT" }, session: { summary: "Session missing", message: "Customer session was not found", code: "SESSION_NOT_FOUND" }, access: { summary: "Invalid customer access token", message: "Invalid customer access token", code: "INVALID_ACCESS_TOKEN" } } }, "403": { description: "Customer is unverified or suspended", message: "Account verification is required", code: "ACCOUNT_VERIFICATION_REQUIRED" }, "423": { description: "Customer account is locked", message: "Account is locked", code: "ACCOUNT_LOCKED" }, "503": { description: "Password change or customer authorization is unavailable", message: "Password change is temporarily unavailable", code: "PASSWORD_CHANGE_UNAVAILABLE" } } },
  ...
  { method: "post", path: "/referrals", tag: "Referrals", summary: "Submit a direct referral", description: "Public with optional customer authentication. Anonymous submissions create or reuse the minimal referral-only identity by normalized phone; customer submissions derive identity from the customer session. The referral and its canonical NEW Admin Lead projection are persisted and linked atomically. Display reference, lifecycle, reward, and payment fields are server-owned.", successStatus: 201, successMessage: "Referral submitted successfully", responseSchema: "DirectReferralResult", security: "optional", bodySchema: "DirectReferralRequest", errorResponses: { "400": { description: "Strict field or identity validation failed", message: "Validation failed", code: "REFERRAL_SUBMISSION_INVALID" }, "429": { description: "Public referral submission rate exceeded", message: "Too many requests, please try again later", code: "REFERRAL_SUBMISSION_RATE_LIMITED" }, "503": { description: "Atomic referral/Lead persistence unavailable", message: "Referral submission is temporarily unavailable", code: "REFERRAL_SUBMISSION_FAILED" } } },
  { method: "post", path: "/referrals/tracking/request", tag: "Referrals", summary: "Request a referral tracking OTP", description: "Referral-only phone verification. Beryl generates and hashes the OTP, then requests delivery through the configured server-side provider. The generic success shape does not disclose whether a phone belongs to a referrer.", successStatus: 202, successMessage: "If the details match, a tracking code will be sent", responseSchema: "ReferralTrackingRequestResult", bodySchema: "ReferralTrackingRequest", errorResponses: { "429": { description: "Rate or resend cooldown exceeded", message: "Please wait before requesting another code", code: "REFERRAL_OTP_RATE_LIMITED" }, "502": { description: "The configured provider did not accept delivery", message: "We could not deliver the referral tracking code", code: "REFERRAL_OTP_DELIVERY_FAILED" }, "503": { description: "No approved/configured delivery provider", message: "Referral tracking by phone is temporarily unavailable", code: "REFERRAL_TRACKING_UNAVAILABLE" } } },
...
const options: swaggerJSDoc.OAS3Options = {
  definition: {
    ...
    tags: [
      ...
      "Admin", "Super Admin", "Dashboard", "Admin Authentication", "Admin Staff Management", "Marketplace Draft Documents", "Marketplace Sales Mandate", "Location Search"
    ].map((name) => ({ name })),
    components,
    paths
  },
