...
const components = {
  ...
  schemas: {
    ...
    MarketplaceInterestRequest: objectSchema({ preferredContactMethod: { type: "string", enum: ["WHATSAPP", "CALL", "EMAIL"] }, message: { type: "string", minLength: 1, maxLength: 1000, nullable: true, description: "Optional Buyer message. Blank-only input is treated as omitted." } }, ["preferredContactMethod"]),
...
const endpoints: Endpoint[] = [
  { method: "get", path: "/marketplace/properties", tag: "Marketplace Public Search", summary: "Search LIVE Marketplace properties", description: "Public anonymous property-card search. The database query enforces marketplace_status=LIVE; legacy approved/is_published records, DRAFT, IN_REVIEW, and REJECTED are excluded. Search and filters use only public fields. Responses are explicitly mapped and never include fullAddress, Seller identity/contact, documents, mandates, review history/feedback, Admin data, or Cloudinary provider identifiers. No saved-state lookup or search analytics is performed in this slice.", successMessage: "Marketplace properties fetched successfully", responseSchema: "MarketplacePublicPropertyList", security: "none", query: [...paginationParameters, queryParameter("q", "Case-insensitive title, description, publicLocation, and propertyType search. Blank values are ignored."), queryParameter("location", "Case-insensitive publicLocation filter only."), queryParameter("minPrice", "Minimum asking price.", { type: "number", minimum: 0 }), queryParameter("maxPrice", "Maximum asking price.", { type: "number", minimum: 0 }), queryParameter("propertyType", "One property type or a comma-separated list, for example APARTMENT,DUPLEX."), queryParameter("category", "Exact Marketplace category.", { type: "string", enum: ["RESIDENTIAL", "COMMERCIAL"] }), queryParameter("bedrooms", "Exact non-negative bedroom count. Commercial properties with null bedrooms do not match.", { type: "integer", minimum: 0 }), queryParameter("sort", "Deterministic Marketplace ordering.", { type: "string", enum: ["DEFAULT", "PRICE_HIGH_TO_LOW", "PRICE_LOW_TO_HIGH", "BEDS", "MOST_RECENT"], default: "DEFAULT" })], errorResponses: { "400": { description: "Filter, price range, sort, page, or limit validation failed", message: "Invalid Marketplace search filters", code: "INVALID_MARKETPLACE_FILTER", examples: { range: { summary: "Minimum exceeds maximum", message: "Invalid Marketplace search filters", code: "INVALID_PRICE_RANGE" }, sort: { summary: "Unknown sort", message: "Invalid Marketplace search filters", code: "INVALID_MARKETPLACE_SORT" }, page: { summary: "Invalid page", message: "Invalid Marketplace search filters", code: "INVALID_PAGE" }, limit: { summary: "Invalid limit", message: "Invalid Marketplace search filters", code: "INVALID_LIMIT" } } }, "503": { description: "Marketplace persistence is unavailable", message: "Marketplace is temporarily unavailable", code: "MARKETPLACE_UNAVAILABLE" } } },
  ...
  { method: "get", path: "/admin/marketplace/properties", tag: "Admin Marketplace Review", summary: "List the Admin Marketplace review queue", description: "Isolated Admin-session operation for ADMIN and SUPER_ADMIN. Defaults to IN_REVIEW, supports database-level lifecycle filtering and pagination, returns counts and deterministic oldest-submission-first review ordering. Customer bearer tokens are not accepted.", successMessage: "Marketplace review queue fetched successfully", responseSchema: "AdminMarketplaceReviewQueue", security: "required", roles: ["ADMIN", "SUPER_ADMIN"], query: [...paginationParameters, queryParameter("status", "Admin review lifecycle filter; defaults to IN_REVIEW.", { type: "string", enum: ["ALL", "IN_REVIEW", "LIVE", "REJECTED"], default: "IN_REVIEW" })], errorResponses: { "400": { description: "Unknown filter or invalid pagination", message: "Invalid review queue filter or pagination", code: "INVALID_REVIEW_QUEUE_FILTER" }, "503": { description: "Review persistence unavailable", message: "Marketplace review queue is temporarily unavailable", code: "MARKETPLACE_REVIEW_UNAVAILABLE" } } },
