...
import type { AgentReport, PublicSupportRepository } from "../src/public-support/repository.js";
...
test("public support reports validate and persist only the supported Agent fields without a session", async t => {
  ...
  let fail = false;
  const repository: PublicSupportRepository = { async submit(report) { if (fail) throw new Error("private SQL detail"); saved.push(report); } };
