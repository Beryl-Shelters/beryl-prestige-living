...
export async function referralsFixture(t:TestContext){
  ...
  for(const migration of ["202609080001_customer_listings.sql","202609100001_short_display_codes.sql","202609130001_customer_referrals.sql"])
  ...
  const repository:ReferralsRepository={list:(id,page,size)=>rpc<Omit<ReferralPage,"program">>("list_customer_referrals",[id,page,size]),create:(id,type,listingId)=>rpc<Omit<CreatedReferral,"referralUrl">>("create_customer_referral_link",[id,type,listingId])};
  ...
  const state={invalid:false};const gateway={readSession:async(hash:string,purpose:string)=>hash===row.token_hash&&purpose===row.purpose&&Date.parse(row.expires_at)>Date.now()?row:null,validate:async()=>{if(state.invalid)throw new Error("invalid provider session");},findCustomer:async()=>profile} as unknown as AuthGateway;
